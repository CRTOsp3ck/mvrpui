            ...
            <VCard>
                <VCardText class="d-flex align-center justify-space-between flex-wrap gap-4">
                    <AppSelect :model-value="itemsPerPage" :items="[
                        ...
                        { value: 100, title: '100' },
                        { value: -1, title: 'All' },
                    ]" style="inline-size: 5rem;" @update:model-value="itemsPerPage = parseInt($event, 10)" />
                ...
                <VDataTableServer v-model:items-per-page="itemsPerPage" v-model:page="page"
                    :items-length="totalPermissions" :items-per-page-options="[
                        { value: 5, title: '5' },
                        { value: 10, title: '10' },
                        { value: -1, title: '$vuetify.dataFooter.itemsPerPageAll' },
                    ]" :headers="headers" :items="permissions" class="text-no-wrap" @update:options="updateOptions">
                    <!-- Assigned To -->
                    <template #item.assignedTo="{ item }">
                        <div class="d-flex gap-2">
                            <VChip v-for="text in item.assignedTo" :key="text" label :color="colors[text].color"
                    ...
                    <template #bottom>
                        ...
                        <div class="d-flex align-center justify-space-between flex-wrap gap-3 pa-5 pt-3">
                            ...
                            <VPagination v-model="page" :length="Math.ceil(totalPermissions / itemsPerPage)"
