...
interface Props {
  grnData: SchemaSaleGoodsReturnNoteView
}
const props = defineProps<Props>()

const emit = defineEmits<{
  (e: 'pushItem', value: SchemaSaleGoodsReturnNoteItemView): void
...
// 👉 Add item function
const addItem = () => {
  const newItem: SchemaSaleGoodsReturnNoteItemView = {
    id: -1,
...
const subtotal = computed(() => {
  return props.grnData.goods_return_note_items.reduce((sum, item) => {
...
<template>
  <VCard>
      ...
      <div class="mt-4 ma-sm-4">
        ...
        <div class="d-flex align-center justify-sm-end mb-3">
          ...
          <span style="inline-size: 9.5rem;">
            <AppDateTimePicker
              v-model="props.grnData.base_document.issue_date"
              density="compact"
        ...
        <div class="d-flex align-center justify-sm-end mb-0">
          ...
          <span style="min-inline-size: 9.5rem;">
            ...
            /> -->
            <AppDateTimePicker
              v-model="props.grnData.return_date"
              density="compact"
              placeholder="YYYY-MM-DD"
              :config="{ position: 'auto right' }"
            />
    ...
    <EntitySearchInput
      v-model="customer"
      :entityId="props.grnData.returned_by_customer_id"
    ...
    <VCardText class="d-flex flex-wrap justify-space-between flex-column flex-sm-row gap-y-5 gap-4">
      ...
      >
        ...
        <p class="mb-1">
          {{ props.grnData.customer_info?.name  }}
        </p>
        <p class="mb-1">
          <!-- {{ customer?.address }}, {{ invoice.client.country }} -->
          {{ props.grnData.customer_info?.address }}
        </p>
        <p class="mb-1">
          {{ props.grnData.customer_info?.phone }}
        </p>
        <p class="mb-0">
          {{ props.grnData.customer_info?.email }}
        </p>
      </div>
          ...
          <tbody>
            <tr>
              ...
              <td class="font-weight-medium pb-1">
                ${{ props.grnData.base_document.net_amount_gen }}
              </td>
            </tr>
    ...
    <VCardText class="add-products-form">
      <div
        v-for="(item, index) in props.grnData.goods_return_note_items"
        :key="item.id"
        class="my-4 ma-sm-4"
      >
