...
import GoodsReturnNoteInfoCard from '../components/GoodsReturnNoteInfoCard.vue'
import CustomerRenderer from '@/pages/documents/sales/components/CustomerRenderer.vue'
import EmployeeRenderer from '@/pages/documents/sales/components/EmployeeRenderer.vue'
import GoodsReturnNoteNumberRenderer from '../components/GoodsReturnNoteNumberRenderer.vue'
...
const colDefs = ref([
  ...
  { 
    field: "goods_return_note_number", 
    headerName:"Ref Number",
    enableRowGroup: true, 
    flex: 1, 
    cellRenderer: GoodsReturnNoteNumberRenderer,
  },
  { 
  ...
  { 
    field: "goods_return_note_items.length", 
    headerName:"Quantity", 
    enableRowGroup: true,  
    enableValue: true, 
    flex: 1,
    valueGetter: (params: any) => { return params.data.goods_return_note_items.length + " Items(s)"; },
  },
  { 
    field: "return_date", 
...
const datasource : IServerSideDatasource = {
  getRows(params) {
    ...
    searchGoodsReturnNoteViewService(req)
    .then((resp) => {
...
<template>
  <div>
    <PageTitle 
      title="Goods Return Notes (GRNs) Management" 
      description="View all goods return notes created over the course of selected period." 
    />
  
    <GoodsReturnNoteInfoCard />
    ...
    <VCard class="mb-6">
      <div class="d-flex align-center justify-between flex-wrap gap-4 mx-6 my-6">
        <!-- 👉 Create button -->
        <VBtn :to="{ name: 'documents-sales-goods-return-note-create-or-update-id', params: { id: '-1' } }" prepend-icon="tabler-plus">
            Create
        </VBtn>
        <!-- 👉 Export button -->
