...
import CustomerRenderer from '../../components/CustomerRenderer.vue'
import ActionRenderer from '../components/ActionRenderer.vue'
import SalesQuotationInfoCard from '../components/SalesQuotationInfoCard.vue'
import SalesQuotationNumberRenderer from '../components/SalesQuotationNumberRenderer.vue'
import SalesQuotationStatusRenderer from '../components/SalesQuotationStatusRenderer.vue';
...
const colDefs = ref([
  ...
  { 
    ...
    enableValue: true, 
    flex: 1,
    valueGetter: (params: any) => { return params.data.sales_quotation_items.length + " Items(s)"; },
  },
  { 
  ...
  { 
    ...
    enableRowGroup: true, 
    flex: 1, 
    cellRenderer: SalesQuotationStatusRenderer,
  },
  { 
    field: "created_at", 
    enableRowGroup: true, 
...
const datasource : IServerSideDatasource = {
  getRows(params) {
    ...
    searchSalesQuotationViewService(req)
    .then((resp) => {
...
<template>
  <div>
    <PageTitle 
      title="Sales Quotations (SQs) Management" 
      description="View all sales quotations created over the course of selected period." 
    />
  
    <SalesQuotationInfoCard />
    ...
    <VCard class="mb-6">
      <div class="d-flex align-center justify-between flex-wrap gap-4 mx-6 my-6">
        <!-- 👉 Create button -->
        <VBtn :to="{ name: 'documents-sales-sales-quotation-create-or-update-id', params: { id: '-1' } }" prepend-icon="tabler-plus">
            Create
        </VBtn>
        <!-- 👉 Export button -->
