...
import SalesInvoiceEditable from '../components/SalesInvoiceEditable.vue'
...
import type { 
  ...
  SchemaDtoUpdateInvoiceItemDto,
  SchemaDtoCreateInvoiceDto,
  SchemaDtoCreateInvoiceItemDto,
  SchemaEntityEntity,
} from '@/composables/schema'
import type { SchemaInvoiceSalesInvoiceItemView, SchemaInvoiceSalesInvoiceView } from '@/composables/ext-schema'

const route = useRoute('documents-sales-sales-invoice-create-or-update-id')

// 👉 Default Blank Data
const salesInvoiceData = ref<SchemaInvoiceSalesInvoiceView>({
  id: -1,
  base_document_id: -1,
  invoice_number: '',
  vendor_id: 1,
...
const getSalesInvoice = async () => {
    ...
    if (resp.status === 200 || resp.status === 201) {
      ...
      salesInvoiceData.value = data.payload
...
const createSalesInvoice = async () => {
  const so: SchemaInvoiceSalesInvoiceView = cloneDeep(salesInvoiceData.value)
  ...
  const createSoItemsDto: SchemaDtoCreateInvoiceItemDto[] = []
  so.invoice_items.forEach((item: SchemaInvoiceSalesInvoiceItemView) => {
      const soItemDto : SchemaDtoCreateInvoiceItemDto = {
  ...
  const createDto: SchemaDtoCreateInvoiceDto = {
  ...
  const resp = await createSalesInvoiceService(createDto)
  if (resp.status === 200 || resp.status === 201) {
  ...
  if (resp.status === 200 || resp.status === 201) {
    setTimeout(async () => {
      const redirectTo = '/documents/sales/sales-invoice/view/' + so.id
...
const updateSalesInvoice = async () => {
  ...
  const updateSoItemsDto: SchemaDtoUpdateInvoiceItemDto[] = []
  so.invoice_items.forEach((item: SchemaInvoiceSalesInvoiceItemView) => {
      const soItemDto : SchemaDtoUpdateInvoiceItemDto = {
          base_document_item: item.base_document_item,
          invoice_item: {
              id: item.id,
              invoice_id: item.invoice_id,
  ...
  const updateDto: SchemaDtoUpdateInvoiceDto = {
    base_document: so.base_document,
    invoice: {
      id: so.id,
      base_document_id: so.base_document_id,
      invoice_number: salesInvoiceData.value.invoice_number,
      vendor_id: so.vendor_id,
      customer_id: so.customer_id,
      payment_due_date: paymentDueDate,
    },
  ...
  const resp = await updateSalesInvoiceService(updateDto)
  if (resp.status === 200 || resp.status === 201) {
  ...
  if (resp.status === 200 || resp.status === 201) {
    setTimeout(async () => {
      const redirectTo = '/documents/sales/sales-invoice/view/' + soId
      await navigateTo({ path: redirectTo })
    }, 1000)
  }
}
...
const addProduct = (value: SchemaInvoiceSalesInvoiceItemView) => {
  salesInvoiceData?.value?.invoice_items?.push(value)
}

const removeProduct = (id: number) => {
  salesInvoiceData?.value?.invoice_items?.splice(id, 1)
}

const updateCustomer = (entity: SchemaEntityEntity) => {
  salesInvoiceData.value.customer_id = entity.id
  salesInvoiceData.value.customer_info = entity
}

const handleCreateOrUpdate = () => {
  salesInvoiceData.value.id && salesInvoiceData.value.id > 0 ? updateSalesInvoice() : createSalesInvoice()
}
...
<template>
  <section>
    ...
    <PageTitleCreateUpdateView
      title="Create / Update Sales Invoice"
      description="Create or update a sales invoice"
