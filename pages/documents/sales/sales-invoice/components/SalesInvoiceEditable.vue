...
import type { SchemaInvoiceSalesInvoiceView, SchemaInvoiceSalesInvoiceItemView } from '@/composables/ext-schema'

interface Props {
  salesInvoiceData: SchemaInvoiceSalesInvoiceView
}
const props = defineProps<Props>()

const emit = defineEmits<{
  (e: 'pushItem', value: SchemaInvoiceSalesInvoiceItemView): void
...
// 👉 Add item function
const addItem = () => {
  const newItem: SchemaInvoiceSalesInvoiceItemView = {
    id: -1,
...
const subtotal = computed(() => {
  return props.salesInvoiceData.invoice_items?.reduce((sum, item) => {
...
<template>
  <VCard>
    ...
    <VCardText class="d-flex flex-wrap justify-space-between gap-y-5 flex-column flex-sm-row">
      ...
      <div class="mt-4 ma-sm-4">
        ...
        <h6 class="d-flex align-center font-weight-medium justify-sm-end text-xl mb-3">
          ...
          <span>
            <AppTextField
              v-model="props.salesInvoiceData.id"
              disabled
              density="compact"
        ...
        <div class="d-flex align-center justify-sm-end mb-3">
          ...
          <span style="inline-size: 9.5rem;">
            <AppDateTimePicker
              v-model="props.salesInvoiceData.base_document.issue_date"
              density="compact"
        ...
        <div class="d-flex align-center justify-sm-end mb-0">
          ...
          <span style="min-inline-size: 9.5rem;">
            ...
            /> -->
            <AppDateTimePicker
              v-model="props.salesInvoiceData.payment_due_date"
              density="compact"
              placeholder="YYYY-MM-DD"
              :config="{ position: 'auto right' }"
            />
    ...
    <EntitySearchInput
      v-model="customer"
      :entityId="props.salesInvoiceData.customer_id"
    ...
    <VCardText class="d-flex flex-wrap justify-space-between flex-column flex-sm-row gap-y-5 gap-4">
      ...
      >
        ...
        <p class="mb-1">
          {{ props.salesInvoiceData.customer_info?.name  }}
        </p>
        <p class="mb-1">
          <!-- {{ customer?.address }}, {{ invoice.client.country }} -->
          {{ props.salesInvoiceData.customer_info?.address }}
        </p>
        <p class="mb-1">
          {{ props.salesInvoiceData.customer_info?.phone }}
        </p>
        <p class="mb-0">
          {{ props.salesInvoiceData.customer_info?.email }}
        </p>
      </div>
          ...
          <tbody>
            <tr>
              ...
              <td class="font-weight-medium pb-1">
                ${{ props.salesInvoiceData.base_document.net_amount_gen }}
              </td>
            </tr>
    ...
    <VCardText class="add-products-form">
      <div
        v-for="(item, index) in props.salesInvoiceData.invoice_items"
        :key="item.id"
        class="my-4 ma-sm-4"
      >
