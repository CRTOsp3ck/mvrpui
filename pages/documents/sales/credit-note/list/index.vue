...
import CreditNoteInfoCard from '../components/CreditNoteInfoCard.vue'
import CustomerRenderer from '@/pages/documents/sales/components/CustomerRenderer.vue'
import CreditNoteNumberRenderer from '../components/CreditNoteNumberRenderer.vue'
...
const colDefs = ref([
  ...
  { 
    ...
    enableRowGroup: true, 
    flex: 1, 
    cellRenderer: CreditNoteNumberRenderer,
  },
  { 
  ...
  { 
    field: "credit_note_items.length", 
    headerName:"Quantity", 
    enableRowGroup: true,  
    enableValue: true, 
    flex: 1,
    valueGetter: (params: any) => { return params.data.credit_note_items.length + " Items(s)"; },
  },
  { 
    field: "issue_date", 
...
const datasource : IServerSideDatasource = {
  getRows(params) {
    ...
    searchCreditNoteViewService(req)
    .then((resp) => {
...
<template>
  <div>
    <PageTitle 
      title="Credit Notes (CNs) Management" 
      description="View all credit notes created over the course of selected period." 
    />
    ...
    <VCard class="mb-6">
      <div class="d-flex align-center justify-between flex-wrap gap-4 mx-6 my-6">
        <!-- 👉 Create button -->
        <VBtn :to="{ name: 'documents-sales-credit-note-create-or-update-id', params: { id: '-1' } }" prepend-icon="tabler-plus">
            Create
        </VBtn>
        <!-- 👉 Export button -->
