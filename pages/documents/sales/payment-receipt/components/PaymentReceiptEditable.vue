...
import PaymentReceiptItemsChild from '../components/PaymentReceiptItemsChild.vue'
import { computed } from 'vue'
import type { SchemaEntityEntity, SchemaInvoicePaymentReceiptItem } from '@/composables/schema'
import type { SchemaInvoicePaymentReceiptView } from '@/composables/ext-schema'

interface Props {
  paymentReceiptData: SchemaInvoicePaymentReceiptView
}
const props = defineProps<Props>()

const emit = defineEmits<{
  (e: 'pushItem', value: SchemaInvoicePaymentReceiptItem): void
...
// 👉 Add item function
const addItem = () => {
  const newItem: SchemaInvoicePaymentReceiptItem = {
    id: -1,
    name: '',
    description: '',
    quantity: 0,
...
const subtotal = computed(() => {
  return props.paymentReceiptData.payment_receipt_items?.reduce((sum, item) => {
...
<template>
  <VCard>
    ...
    <VCardText class="d-flex flex-wrap justify-space-between gap-y-5 flex-column flex-sm-row">
      ...
      <div class="mt-4 ma-sm-4">
        ...
        <h6 class="d-flex align-center font-weight-medium justify-sm-end text-xl mb-3">
          ...
          <span>
            <AppTextField
              v-model="props.paymentReceiptData.id"
              disabled
              density="compact"
        ...
        <div class="d-flex align-center justify-sm-end mb-3">
          ...
          <span style="inline-size: 9.5rem;">
            <AppDateTimePicker
              v-model="props.paymentReceiptData.date_of_payment"
              density="compact"
    ...
    <EntitySearchInput
      v-model="customer"
      :entityId="props.paymentReceiptData.payer_id"
    ...
    <VCardText class="d-flex flex-wrap justify-space-between flex-column flex-sm-row gap-y-5 gap-4">
      <div v-if="customer" class="ma-sm-4" style="inline-size: 15.5rem;">
        ...
        <p class="mb-1">
          {{ props.paymentReceiptData.payer_info?.name  }}
        </p>
        <p class="mb-1">
          <!-- {{ customer?.address }}, {{ invoice.client.country }} -->
          {{ props.paymentReceiptData.payer_info?.address }}
        </p>
        <p class="mb-1">
          {{ props.paymentReceiptData.payer_info?.phone }}
        </p>
        <p class="mb-0">
          {{ props.paymentReceiptData.payer_info?.email }}
        </p>
      </div>
    ...
    <VCardText class="add-products-form">
      <div
        v-for="(item, index) in props.paymentReceiptData.payment_receipt_items"
        :key="item.id"
        class="my-4 ma-sm-4"
      >
        <PaymentReceiptItemsChild
          :id="index"
          :data="item"
