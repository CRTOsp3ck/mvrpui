...
import ReturnMerchandiseAuthorizationNumberRenderer from '../components/ReturnMerchandiseAuthorizationNumberRenderer.vue'
import ReturnMerchandiseAuthorizationInfoCard from '../components/ReturnMerchandiseAuthorizationInfoCard.vue'
...
const colDefs = ref([
  ...
  { 
    ...
    enableRowGroup: true, 
    flex: 1, 
    cellRenderer: ReturnMerchandiseAuthorizationNumberRenderer,
  },
  { 
    field: "returned_by_info.name", 
...
const datasource : IServerSideDatasource = {
  getRows(params) {
    ...
    searchReturnMerchandiseAuthorizationViewService(req)
    .then((resp) => {
...
<template>
  <div>
    <PageTitle 
    title="Return Merchandise Authorizations (RMAs) Management" 
    description=" View all return merchandise authorizations created over the course of selected period." 
    />
  
    <ReturnMerchandiseAuthorizationInfoCard />
    ...
    <VCard class="mb-6">
      <div class="d-flex align-center justify-between flex-wrap gap-4 mx-6 my-6">
        <!-- 👉 Create button -->
        <VBtn :to="{ name: 'documents-inventory-return-merchandise-authorization-create-or-update-id', params: { id: '-1' } }" prepend-icon="tabler-plus">
            Create
        </VBtn>
        <!-- 👉 Export button -->
